...
if (!class_exists('ESIG_WOOCOMMERCE_Admin')) :
    ...
    class ESIG_WOOCOMMERCE_Admin {
        ...
        public function woo_init() {
            ...
            add_action('woocommerce_new_order', array($this, 'esig_after_checkout_form'), 10, 1);
            add_action('woocommerce_thankyou', array($this, 'esig_checkout_after'), 10, 1);
            add_action('woocommerce_order_status_pending_to_on-hold_notification', array($this, 'esig_new_woo_order'));
            add_action('esig_document_before_display', array($this, 'esig_signature_after'), 10, 1);
        ...
        public function esig_after_checkout_form($order_id) {
            ...
            if (sizeof($woocommerce->cart->cart_contents) > 0) {
                foreach ($woocommerce->cart->get_cart() as $cart_item) {
                    ...
                    if ($agreement_logic == 'after_checkout' && $agreement_signed == 'no') {
                        $agreement_list[$esig_agreement['agreement_id']] = 'no';
                    }
                }
            }
            ...
            if ($global_document_id) {
                $agreement_list[$global_document_id] = 'no';
            }
        ...
        public function esig_checkout_after($order_id) {
            ...
            foreach ($doc_list as $document_id => $value) {
                if ($value == "no") {
                    $sad_page = esig_woo_logic::get_sad_page_id($document_id);
                    break;
                }
            }
            if ($sad_page) {
        ...
        public function esig_signature_after($args) {
            ...
            if (esig_woo_logic::get_after_checkout_order_id()) {
                $result = $this->after_checkout_signed_update($sad_doc_id, $document_id);
                if ($result) {
        ...
        public function after_checkout_signed_update($sad_doc_id, $document_id) {
